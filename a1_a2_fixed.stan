...
model {
  ...
  y ~ bernoulli((1 - (1 - prob_detect)^effort) .* (1 / (1 + exp(-exp(log_a1[year]) .* (jj_date - b1))) .* (1 / (1 + exp(exp(log_a2[year]) .* (jj_date - b2))))));
}
generated quantities {
  vector[n_new*N_Y] mu_line = (1 / (1 + exp(-exp(log_a1[newyear]) .* (newdate - b1)))) .* (1 / (1 + exp(exp(log_a2[newyear]) .* (newdate - b2))));
}
