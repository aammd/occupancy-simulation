...
It is not really surprising that the model still work as we just added data and different values of $b1$ for those slices of data. We could do this for all parameters, but it will end up with the same results that we had in our previous analysis with all parameters.

The next step is to test for all parameters.

## Two b parameter varying with year

This part of the analysis work well and we are able to get back our simulated parameter values
...
Run the model and inspect the posterior parameter prediction
...
This simulation works also very well and the posterior predict with good accuracy our simulated parameters
...
Here is where things gets messy. When we make both $a$ parameters vary with years, the model gets confuse and is no longer able to produce a decent prediction. We show it here :

Visualize the simulated data. Note that the slope changes each year for both $a$.
...
Inspect the posterior. Both the sd and rhat show us that this model is horrible. 
...
Our new fake data set looks like this. Note the presence of only one parameter $a$ and the new parameter $f$.
...
We plot the fake data as before. These looks just like the one when the two $a$ changed. 
...
We run the model and inspect the posterior. 
...
And finally we confirm that everything is good by ploting the predicted regression for each years. 
...
We will test our model with the LTDU data. At each year, one set of parameter is estimated (not hierarchical, yet). 
...
Plot of the predicted regression. It works very well! But as we can see, the prediction for 2021 is not very good as we have no data. 
...
w &= 1 - (1 - d)^{effort} \\
p &= \dfrac{e^{-a^i f^i (\text{date} - b_1^i)}}{1 + e^{-a^i f^i (date - b_1^i)}} * \dfrac{e^{a^i (1 - f^i) (date - b_2^i)}}{1+e^{a^i (1 - f^i) (date - b_2^i)}}\\\\ 
...
Here is the pdf of all the parameters
...
But when we try to run this model, things get bad. The r-hat are good, but when we look at the predicted graph, we see that the variance is some parameters is to high + it seems the model have some difficulties with parameter identification (maybe it confuse $b1$ and $b1$). 
...
```{r}
#| echo: false
tar_load(b1_hierarchical_mcmc_b1_hierarchical)

b1_hierarchical_mcmc_b1_hierarchical$draws(variables = c("prob_detect", "log_a", "f", "b1", "b2", "b1_avg", "b1_sigma")) |> 
  posterior::summarise_draws("mean", "sd", ~quantile(.x, probs = c(0.05, 0.95)), "rhat") |> 
  flextable::flextable()
...
b1_hierarchical_mcmc_b1_hierarchical |> 
  tidybayes::gather_rvars(mu_line[i]) |> 
  dplyr::bind_cols(as.data.frame(fake_data_hierarchical[c("newdate", "newyear")])) |> 
  ggplot(aes(x = newdate, dist = .value, group = newyear)) + 
  tidybayes::stat_lineribbon() +
  theme_classic() +
  facet_wrap(~ newyear)
...
### b1 hierarchical normal centered

One solution would be to center the parameter and scale it back for the prediction. Unfortunately, it doesn't help that much. 
...
tar_load(b1_hierarchical_nc_mcmc_b1_hierarchical_nc)

b1_hierarchical_nc_mcmc_b1_hierarchical_nc$draws(variables = c("prob_detect", "log_a", "f", "b1", "b2", "b1_avg", "b1_sigma")) |> 
  posterior::summarise_draws("mean", "sd", ~quantile(.x, probs = c(0.05, 0.95)), "rhat") |> 
  flextable::flextable()
...
b1_hierarchical_nc_mcmc_b1_hierarchical_nc |> 
  tidybayes::gather_rvars(mu_line[i]) |> 
  dplyr::bind_cols(as.data.frame(fake_data_hierarchical[c("newdate", "newyear")])) |> 
  ggplot(aes(x = newdate, dist = .value, group = newyear)) + 
  tidybayes::stat_lineribbon() +
...
Why not trying with all the parameters hierarchical to see if it works better? (spoiler alert: it does!)
...
all_hier_stan <- cmdstanr::cmdstan_model(stan_file = "all_hierarchical.stan")
...
We try to give flexible prior so not to force to much the arrival and departure date and the slopes. Note that the prior for $b1_sigma$ and $b2_sigma$ are identical (but are two seperate parameters). 
...
ggplot(data.frame(x = c(110, 260)), aes(x = x)) +
  xlim(c(110, 260)) +
  stat_function(fun = dnorm, args = list(mean = 150, sd = 10), geom = "area", aes(fill = "b1_avg"), lwd = 1.5) +
  stat_function(fun = dnorm, args = list(mean = 220, sd = 10), geom = "area", aes(fill = "b2_avg"), lwd = 1.5) +
  scale_fill_manual("Parameters", values = alpha(c(viridis::viridis(7)[1], viridis::viridis(7)[2]), 0.7)) +
  labs(x = "Julian date", y = "Prob. dens.") +
  theme_classic()

ggplot(data.frame(x = c(0, 60)), aes(x = x)) +
  xlim(c(0, 20)) +
  stat_function(fun = dcauchy, args = list(location = 0, scale = 7), geom = "area", aes(fill = "b1_sigma"), lwd = 1.5) +
  scale_fill_manual("Parameters", values = alpha(c(viridis::viridis(7)[3]), 0.7)) +
  labs(x = "sigma", y = "Prob. dens.") +
  theme_classic()

ggplot(data.frame(x = c(-4, 3)), aes(x = x)) +
  xlim(c(-4, 3)) +
  stat_function(fun = dnorm, args = list(mean = -1, sd = 0.8), geom = "area", aes(fill = "log_a_avg"), lwd = 1.5) +
  stat_function(fun = dnorm, args = list(mean = 1, sd = 0.5), geom = "area", aes(fill = "f_avg"), lwd = 1.5) +
  scale_fill_manual("Parameters", values = alpha(c(viridis::viridis(7)[4], viridis::viridis(7)[5]), 0.7)) +
  labs(x = "log slope", y = "Prob. dens.") +
  theme_classic()

ggplot(data.frame(x = c(0, 5)), aes(x = x)) +
  xlim(c(0, 5)) +
  stat_function(fun = dcauchy, args = list(location = 0, scale = 1), geom = "area", aes(fill = "log_a_sigma"), lwd = 1.5) +
  stat_function(fun = dcauchy, args = list(location = 0, scale = 0.5), geom = "area", aes(fill = "f_sigma"), lwd = 1.5) +
  scale_fill_manual("Parameters", values = alpha(c(viridis::viridis(7)[6], viridis::viridis(7)[7]), 0.7)) +
  labs(x = "sigma", y = "Prob. dens.") +
  theme_classic()
...
The model works well! All parameters are estimated with reasonable variance and the r-hat values are good.
...
tar_load(all_hierarchical_mcmc_all_hierarchical)

all_hierarchical_mcmc_all_hierarchical$draws(variables = c("prob_detect", "log_a", "f", "b1", "b2", "b1_avg", "b1_sigma", "b2_avg", "b2_sigma",
"log_a_avg", "log_a_sigma", "f_avg", "f_sigma")) |> 
  posterior::summarise_draws("mean", "sd", ~quantile(.x, probs = c(0.05, 0.95)), "rhat") |> 
  flextable::flextable()
...
##### Plot of the predicted relation

All of the regression curves look good!
```{r}
#| echo: false
...
all_hierarchical_mcmc_all_hierarchical |> 
  tidybayes::gather_rvars(mu_line[i]) |> 
  dplyr::bind_cols(as.data.frame(fake_data_hierarchical[c("newdate", "newyear")])) |> 
  ggplot(aes(x = newdate, dist = .value, group = newyear)) + 
  tidybayes::stat_lineribbon() +
...
# Testing the hierarchical model with duck data

We lunch the model on the long-tailed duck data. We expect little variation in the $b$ parameters and a great variance for 2021 as there is no data in the spring (and little data at all). 

The model seems to perform just fine! 
...
tar_load(duck_hierarchical_mcmc_all_hierarchical)

duck_hierarchical_mcmc_all_hierarchical$draws(variables = c("prob_detect", "log_a", "f", "b1", "b2", "b1_avg", "b1_sigma", "b2_avg", "b2_sigma",
"log_a_avg", "log_a_sigma", "f_avg", "f_sigma")) |> 
  posterior::summarise_draws("mean", "sd", ~quantile(.x, probs = c(0.05, 0.95)), "rhat") |> 
  flextable::flextable()
...
Graphical examination of the predicted regression. 
...
duck_hierarchical_mcmc_all_hierarchical |> 
  tidybayes::gather_rvars(mu_line[i]) |> 
  dplyr::bind_cols(as.data.frame(duck_data_all[c("newdate", "newyear")])) |> 
  ggplot(aes(x = newdate, dist = .value, group = newyear)) + 
  tidybayes::stat_lineribbon() +
...
Plot of the difference between $b1$ and $b2$ values for each year. First try to look at the difference not only in arrival and departure date, but also in duration. 

```{r}
#| echo: false

duck_hierarchical_mcmc_all_hierarchical |>
  tidybayes::gather_rvars(stay[i]) |> 
...
# Testing the hierarchical model with Lapland longspur data

We lunch the model on a second species not used so far to develop the model to validate its generality on other species. It is interesting to note that the departure date estimated is after the last observation as often one or two individuals are observed at the very end of the season. 
...
tar_load(lalo_hierarchical_mcmc_all_hierarchical)

lalo_hierarchical_mcmc_all_hierarchical$draws(variables = c("prob_detect", "log_a", "f", "b1", "b2", "b1_avg", "b1_sigma", "b2_avg", "b2_sigma",
"log_a_avg", "log_a_sigma", "f_avg", "f_sigma")) |> 
  posterior::summarise_draws("mean", "sd", ~quantile(.x, probs = c(0.05, 0.95)), "rhat") |> 
  flextable::flextable()
...
As expected, the departure is very badly explained. The slope goes way beyond the date at which everyone left Bylot. Of course, the arrival of winter would force the specie to leave, so the estimated departure date is probably wrong. 
...
tar_load(lalo_data_all)

lalo_hierarchical_mcmc_all_hierarchical |> 
  tidybayes::gather_rvars(mu_line[i]) |> 
  dplyr::bind_cols(as.data.frame(lalo_data_all[c("newdate", "newyear")])) |> 
  ggplot(aes(x = newdate, dist = .value, group = newyear)) + 
  tidybayes::stat_lineribbon() +
  theme_classic() +
  facet_wrap(~ newyear)
...
- [X] Simulate data with a hierarchical structure for one parameters (ex: b1)
- [X] Run a hierarchical model for one parameters
- [X] Simulate data with a hierarchical structure for all parameters
- [X] Run a hierarchical model for all parameters
- [X] Test it with LTDU data
- [X] Celebrate 🥳
